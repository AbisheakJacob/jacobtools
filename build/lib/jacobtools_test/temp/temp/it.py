...
def combo(df_data, column_name, groupby_column, combo_no):
    ...
    new_data["all_combinations"] = new_data[column_name].apply(
        lambda x: list(combinations(x, combo_no))
    )

    # count the number of occurances of each particular combinations

    combinations_counter = Counter(
        [tuple(sorted(i)) for sublist in new_data["all_combinations"] for i in sublist]
    )
