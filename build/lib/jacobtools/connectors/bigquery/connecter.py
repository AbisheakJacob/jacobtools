...
from your_package.connectors.base import BaseConnector
from your_package.config.settings import BigQuerySettings
from your_package.exceptions.errors import ConfigurationError
from your_package.utils.validation import validate_table_reference

# Import the specialized managers
from your_package.connectors.bigquery.client import BigQueryClientWrapper
from your_package.connectors.bigquery.query import QueryManager
from your_package.connectors.bigquery.metadata import MetadataManager
from your_package.connectors.bigquery.profiling import ProfilerManager
...
class GoogleBigQueryConnector(BaseConnector):
    ...
    def __init__(self, project_id: Optional[str] = None, credentials_path: Optional[str] = None):
        ...
        self._client_wrapper = BigQueryClientWrapper(self.project_id, self.credentials_path)
        self._query_manager = QueryManager(self._client_wrapper)
        self._metadata_manager = MetadataManager(self._client_wrapper, self._query_manager)
    ...
    def write_data(self, df: pd.DataFrame, dataset_id: str, table_id: str, if_exists: str = "append") -> None:
        validate_table_reference(self.project_id, dataset_id, table_id)
